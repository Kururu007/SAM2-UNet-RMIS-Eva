...
if __name__ == "__main__":
    ...
    temp_parameters_name_train = {}
    temp_parameters_name_fixed = {}
    
    for k, v in model.named_parameters():
        if v.requires_grad:
            train.append(v)
            temp_parameters_name_train[k] = v
        else:
            fixed.append(v)
            temp_parameters_name_fixed[k] = v
            
    n_train_parameters = sum(p.numel() for p in train)
    print(f'number of updated params (Trainable): {n_train_parameters}.')
    n_fixed_parameters = sum(p.numel() for p in fixed)
    print(f'number of updated params (Fixed): {n_fixed_parameters}.')
